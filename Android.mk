...
RECOVERY_NAME := Sk8s CWMRTouch for Xperia
...
TARGET_DEVICE := m7
LOCAL_CFLAGS += -DTARGET_DEVICE_M7 -DTARGET_DEVICE_768
...
# LG Optimus G
else ifeq ($(TARGET_PRODUCT), full_geeb)
TARGET_DEVICE := geeb
BOARD_USE_CUSTOM_RECOVERY_FONT := \"roboto_15x24.h\"
LOCAL_CFLAGS += -DTARGET_DEVICE_E970 -DTARGET_DEVICE_768

# LG Optimus G
else ifeq ($(TARGET_PRODUCT), full_geebus_bell)
TARGET_DEVICE := geebus_bell
BOARD_USE_CUSTOM_RECOVERY_FONT := \"roboto_15x24.h\"
LOCAL_CFLAGS += -DTARGET_DEVICE_E973 -DTARGET_DEVICE_768
...
# Viewsonic G-Tablet
else ifeq ($(TARGET_PRODUCT), cm_smba1002)
TARGET_DEVICE := smba1002
BOARD_USE_CUSTOM_RECOVERY_FONT := \"roboto_15x24.h\"
LOCAL_CFLAGS += -DTARGET_DEVICE_SMBA1002 -DTARGET_DEVICE_600
...
include $(CLEAR_VARS)
LOCAL_MODULE := no-assert.sh
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE_CLASS := RECOVERY_EXECUTABLES
LOCAL_MODULE_PATH := $(TARGET_RECOVERY_ROOT_OUT)/sbin
LOCAL_SRC_FILES := no-assert.sh
include $(BUILD_PREBUILT)
